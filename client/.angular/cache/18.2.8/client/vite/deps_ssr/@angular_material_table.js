...
} from "./chunk-3MUFASZP.js";
import {
  DOCUMENT
} from "./chunk-7IGVRCUA.js";
import {
  Attribute,
...
var CdkTable = class _CdkTable {
  ...
  constructor(_differs, _changeDetectorRef, _elementRef, role, _dir, _document, _platform, _viewRepeater, _coalescedStyleScheduler, _viewportRuler, _stickyPositioningListener, _unusedNgZone) {
    ...
    this._onDestroy = new import_rxjs2.Subject();
    ...
    this.viewChange = new import_rxjs2.BehaviorSubject({
      start: 0,
  ...
  ngOnInit() {
    ...
    this._viewportRuler.change().pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe(() => {
      this._forceRecalculateCellWidths = true;
    });
  }
  ngAfterContentInit() {
  ...
  _observeRenderChanges() {
    ...
    } else if ((0, import_rxjs2.isObservable)(this.dataSource)) {
      dataStream = this.dataSource;
    } else if (Array.isArray(this.dataSource)) {
      dataStream = (0, import_rxjs2.of)(this.dataSource);
    }
    if (dataStream === void 0 && (typeof ngDevMode === "undefined" || ngDevMode)) {
      throw getTableUnknownDataSourceError();
    }
    this._renderChangeSubscription = dataStream.pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe((data) => {
      this._data = data || [];
      this.renderRows();
    });
  }
  ...
  _setupStickyStyler() {
    ...
    (this._dir ? this._dir.change : (0, import_rxjs2.of)()).pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe((value) => {
...
var MatTableDataSource = class extends DataSource {
  ...
  constructor(initialData = []) {
    super();
    this._renderData = new import_rxjs3.BehaviorSubject([]);
    this._filter = new import_rxjs3.BehaviorSubject("");
    this._internalPageChanges = new import_rxjs3.Subject();
    ...
    this._data = new import_rxjs3.BehaviorSubject(initialData);
    this._updateChangeSubscription();
  }
  /**
  ...
  _updateChangeSubscription() {
    const sortChange = this._sort ? (0, import_rxjs3.merge)(this._sort.sortChange, this._sort.initialized) : (0, import_rxjs3.of)(null);
    const pageChange = this._paginator ? (0, import_rxjs3.merge)(this._paginator.page, this._internalPageChanges, this._paginator.initialized) : (0, import_rxjs3.of)(null);
    const dataStream = this._data;
    const filteredData = (0, import_rxjs3.combineLatest)([dataStream, this._filter]).pipe((0, import_operators3.map)(([data]) => this._filterData(data)));
    const orderedData = (0, import_rxjs3.combineLatest)([filteredData, sortChange]).pipe((0, import_operators3.map)(([data]) => this._orderData(data)));
    const paginatedData = (0, import_rxjs3.combineLatest)([orderedData, pageChange]).pipe((0, import_operators3.map)(([data]) => this._pageData(data)));
